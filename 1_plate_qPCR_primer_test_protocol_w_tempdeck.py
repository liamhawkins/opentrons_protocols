...
tube_rack_2ml = labware.load('opentrons-aluminum-block-2ml-eppendorf', '8')
small_reagent_plate = labware.load('PCR-strip-tall', '6')
tempdeck = modules.load('tempdeck', '10')
pcr_plate1 = labware.load('opentrons-aluminum-block-96-PCR-plate', '10', share=True)
...
def run_custom_protocol(number_of_mixing: int = 5, mix_rate: int = 6):
    ...
    buffer_mix_tube = tube_rack_2ml.wells('A2')  # Water, Trehalose, qPCR Buffer
    high_vol_buffer_mix_tube = (buffer_mix_tube, buffer_mix_tube.from_center(x=0, y=0, z=0))

    # Separate master mix tubes for each primer
    master_mix_tubes = [well.bottom() for well in tube_rack_2ml.wells('D1', 'D2', 'D3', 'D4', 'D5', 'C5')]
    ...
    for master_mix, column in zip(master_mix_tubes, list(range(1, 12 + 1, 2))):
        ...
        p50_multi.distribute(MASTER_MIX_VOL,
                             master_mix,
                             master_mix_dispense_location(pcr_plate1, first_column, second_column),
                             disposal_vol=0,
                             blow_out=True)
